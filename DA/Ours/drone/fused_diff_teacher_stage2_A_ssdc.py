...
work_dir = f'work_dirs/stage2-{timestamp}'  # 自动变成 'work_dirs/stage2-1222'

_base_ = [  # 继承基础配置列表
    '../../_base_/models/diffusion_guided_adaptation_faster_rcnn_r101_fpn.py',  # 基础扩散-检测模型结构
    '../../_base_/da_setting/semi_20k.py',  # 20000迭代半监督/域自适应训练日程
    '../../_base_/datasets/sim_to_real/semi_drone_rgb_aug.py',  # 仿真 RGB → 真实 RGB 数据集定义
]  # 结束基础配置
burn_cross = 20000  # 定义burn_up_iters以便后续引用
burn_ssdc = 30000  # 定义SS-DC烧入步数以便后续引用
...
ssdc_runtime_cfg = dict(  # 整理SS-DC运行期配置以匹配SSDCFasterRCNN签名
    ...
    w_decouple=[(0, 0.1), (6000, 0.3)],  # 复制阶段性解耦权重调度
    # w_couple=[(2000, 0.2), (10000, 0.5)],  # 复制阶段性耦合权重调度
    w_couple=[(0, 0.0), (4000, 0.01), (5000, 0.05), (12000, 0.1)],  # 修改耦合权重调度以适应烧入期
    w_di_consistency=[(0, 0.0), (4000, 0.01), (5000, 0.02), (12000, 0.08)],  # 设置域不变一致性损失权重
    # consistency_gate=[(0, 0.3), (12000, 0.5)],  # 设置伪标签余弦门限调度
    consistency_gate=[(0, -1.0), (4000, -1.0), (8000, 0.01), (15000, 0.1),(18000, 0.3)],  # 修改余弦门限调度以适应烧入期
...
custom_hooks = [
    ...
    dict(
        ...
        momentum=0.0004,
        interval=1,
        burn_up_iters=40000,  # 让 student 先预热，再开始更新 diff_teacher head（你要的预热逻辑）
