...
from typing import Dict, List, Sequence, Tuple  # 导入类型注解提升可读性与IDE提示
...
class SSDCouplingNeck(nn.Module):  # 定义光谱-空间耦合颈部模块
    ...
    def forward(self,
        ...
        for idx, (spatial, inv, ds) in enumerate(zip(spatial_feats, inv_feats, ds_feats)):  # 同步遍历三个特征列表
            ...
            attn_output = attn_output.view(spatial.shape[0], self.in_channels, spatial.shape[2], spatial.shape[3])  # 将注意力输出恢复为原始形状
